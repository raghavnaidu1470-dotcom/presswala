      ...
      <div style={{ padding: '0.25rem 0' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem' }}>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderRadius: '16px',
                  background: currentUser?.flat_number === 'VENDOR' ? 'rgba(123, 174, 92, 0.12)' : '#F6F5F2',
                  border: currentUser?.flat_number === 'VENDOR' ? '2px solid var(--primary-500)' : '1px solid rgba(0, 0, 0, 0.06)',
                  cursor: 'pointer',
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--text-primary)' }}>Ramu Dhobi (Active Vendor)</div>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Palm Heights • Deliveries & price list</div>
                  </div>
                </div>
                <span className="badge badge-vendor">Active</span>
              </div>
            );
          })()}
          ...
          {(() => {
            ...
            return (
              <div
                onClick={() => handleSelectUser(suresh)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'space-between',
                  padding: '0.9rem 1.1rem',
                  borderRadius: '16px',
                  background: currentUser?.flat_number === 'VENDOR-SURESH' ? 'rgba(212, 163, 115, 0.15)' : '#F6F5F2',
                  border: currentUser?.flat_number === 'VENDOR-SURESH' ? '2px solid #D4A373' : '1px solid rgba(0, 0, 0, 0.06)',
                  cursor: 'pointer',
                  transition: 'all 150ms ease'
                }}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <div style={{ width: '38px', height: '38px', borderRadius: '10px', background: 'rgba(212, 163, 115, 0.2)', color: '#8F6235', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <ShieldCheck size={20} />
                  </div>
                  <div>
                    <div style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--text-primary)' }}>Suresh Laundry (Pending)</div>
                    <div style={{ fontSize: '0.8rem', color: '#8F6235' }}>Green Glen Villas • Pending approval</div>
                  </div>
                </div>
                <span className="badge" style={{ background: 'rgba(212, 163, 115, 0.2)', color: '#8F6235', fontWeight: 700 }}>Pending</span>
              </div>
            );
          })()}
          ...
          {(() => {
            ...
            return (
              <div
                onClick={() => handleSelectUser(pooja)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'space-between',
          ...
          {(() => {
            ...
            return (
              <div
                onClick={() => handleSelectUser(sharma)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'space-between',
          ...
          {(() => {
            const karthik = INITIAL_USERS.find(u => u.flat_number === 'B-3002');
            if (!karthik) return null;
            return (
              <div
                onClick={() => handleSelectUser(karthik)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'space-between',
